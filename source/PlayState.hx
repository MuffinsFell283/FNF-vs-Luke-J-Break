...
class PlayState extends MusicBeatState
{
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:String = checkForAchievement(['weekluke_nomiss', 'weekluker_nomiss', 'cover_nomiss', 'ur_bad',
	...
	private function checkForAchievement(achievesToCheck:Array<String> = null):String
	{
		...
		for (i in 0...achievesToCheck.length) {
			...
			if(!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled) {
				...
				{
					case 'weekluke_nomiss' | 'weekluker_nomiss':
						if(isStoryMode && campaignMisses + songMisses < 1 && CoolUtil.difficultyString() == 'OCEAN' && storyPlaylist.length <= 1 && !changedDifficulty && !usedPractice)
						{
							var weekName:String = WeekData.getWeekFileName();
							switch(weekName) //I know this is a lot of duplicated code, but it's easier readable and you can add weeks with different names than the achievement tag
							{
								case 'weekLuke':
									if(achievementName == 'weekluke_nomiss') unlock = true;
									if(achievementName == 'weekluker_nomiss') unlock = true;
							}
						}
					case 'ur_bad':
					...
					case 'cover_nomiss':
						if(Paths.formatToSongPath(SONG.song) == 'ecstasy' && !usedPractice && !changedDifficulty && CoolUtil.difficultyString() == 'HARD' && campaignMisses + songMisses < 1){
							unlock = true;
						}
				}
